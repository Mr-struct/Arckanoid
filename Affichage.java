...
public class Affichage extends JPanel {
	...
	public Affichage (int width,int height,Modele modele) {
		...
		// on charge les image 
		try {
			imgBackground = ImageIO.read(new File("src/F1.jpg"));
			
			imgDefaultRaquette = ImageIO.read(new File("src/bar1.png"));
			
			imgDefaultBalle = ImageIO.read(new File("src/BALLE2.png"));
			
			for(int i = 0 ; i< 5;i++) {
				
				imgBrique.add( ImageIO.read(new File("src/br"+i+".png")) );
	...
	public void paintComponent(Graphics g) {
		...
		//dessine la balle 
		g2d.drawImage(imgDefaultBalle, modele.balle.getX(),modele.balle.getY(),20,20, this);
		...
		g2d.drawString("Level 0",this.getWidth()-170 , 120);
		g2d.drawString("THE WAY",this.getWidth()-170 , 160);
		...
		g2d.setColor(Color.MAGENTA);
		g2d.fillRect(this.getWidth()-1000, 0, 10, this.getHeight());//droite
		g2d.fillRect(this.getWidth()-200, 0, 10, this.getHeight());//gauche
		//getWidth
