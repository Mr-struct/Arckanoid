...
public class PanelGame extends JPanel{
	...
	private Image imgDefaultRaquette,imgDefaultBalle;

	private  ArrayList<Image> imgBrique = new ArrayList<Image>(0);

	private ArrayList<Image> imgBonus = new ArrayList<Image>(0);

	private ImageIcon fier1,fier2,fier3,thunder ,imgConfettis, imgBackground1;
	...
	public PanelGame (int width,int height,Vue vue) {
		...
		ControleurJeu ctrlGame = new ControleurJeu(this.vue.modele,this.vue);
	...
	public void paintComponent(Graphics g) {
		...
		synchronized (vue.modele.balles){
			for(Balle balle: vue.modele.balles) {
				synchronized(ballsTrac) {
					ballsTrac.add (new AnimatedObject(balle.getIntX(),balle.getIntY(),balle.getWidth(),balle.getHeight()));
				}
			}
		}
		...
		synchronized(ballsTrac) { for(AnimatedObject trac : ballsTrac) {
			/*
			 * en parler avec stephane 
			 * savoir si il n 'y a pas d'interblocage avec le modele 
			 */
/* a decomneter pour avoir les effet de lumer sur la balle
			g2d.setColor(new Color(255,255,255,80-trac.getAge()));
			g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
			g2d.fillOval((trac.getX() + trac.getAge()/2)-10,
					(trac.getY() + trac.getAge()/2)-10,
					trac.getWidth()+20,trac.getHeight()+20);
			g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_OFF);
*/
		...
		GradientPaint gp1 = new GradientPaint(vue.modele.gameWidth-100, 0, transparentColor3, vue.modele.gameWidth-100, vue.modele.gameHeight, transparentColor4, true);                
		g2d.setPaint(gp1);
		g2d.fillRect((vue.modele.gameWidth-200), 0, 200, vue.modele.gameHeight);

		//degrade de couleur
		GradientPaint gp2 = new GradientPaint(100, 0, transparentColor3, 100, vue.modele.gameHeight, transparentColor4, true);                
		g2d.setPaint(gp2);
		...
		*/
		...
		vue.pauseButton.setBounds(this.getWidth()-this.getWidth()/8,this.getHeight()/2-this.getHeight()/6,64,64);
		vue.soundButton.setBounds(this.getWidth()-this.getWidth()/8,this.getHeight()/2-this.getHeight()/20,64,64);
		...
		if(vue.modele.win) {
			...
			vue.pauseButton.setBounds(-this.getWidth(), -this.getHeight(),0,0);
			vue.soundButton.setBounds(-this.getWidth(), - this.getHeight(),0,0);
			...
			if(vue.modele.lose) {
				...
				if(!vue.modele.isRunning()) {
					...
					g2d.fillRect(0,0,vue.modele.gameWidth,vue.modele.gameHeight);
					//dégradé1 de couleur
					GradientPaint gp5 = new GradientPaint(200 + (vue.modele.gameWidth-400)/2, 
							0, transparentColor1, 
							200 + (vue.modele.gameWidth-400)/2,														vue.modele.gameHeight, 
							transparentColor2, true);                
					g2d.setPaint(gp5);
					g2d.fillRect(200,0, vue.modele.gameWidth-400, vue.modele.gameHeight);
					...
					g2d.setTransform(oldTransform);
					int x = this.getWidth()/2;
					int y = this.getHeight()/2 + this.getHeight()/8;
					vue.backLevelSelectButton.setBounds(x-this.getWidth()/8 - 64, y, 64, 64);
					vue.crossButton.setBounds(x+this.getWidth()/8,y, 64, 64);
					vue.pauseButton.setBounds(-this.getWidth(), -this.getHeight(),0,0);
					vue.soundButton.setBounds(-this.getWidth(), - this.getHeight(),0,0);
